...
router.get('/', async (req, res) => {
  ...
  const dbData = file.MsgBody.items.map((values)=>{
  ...
  db.query(`select animalSeq from lostBoard`, (err, results)=>{
    results.sort((a,b)=>{
  ...
  db.query(`select * from lostBoard`, (err, results)=>{
    ...
    let fileImage = results.map((values, index)=>{
      ...
      if(index % 10 == 0){
        return `
      <div class="imbox"><img alt="이미지 준비중" src="${imageURL + values.filePath}"/>`
      }
      else if(index == results.length){
        return `
      <img alt="이미지 준비중" src="${imageURL + values.filePath}"/></div>`
      }
      else if(index % 10 == 9){
        return `
      <img alt="이미지 준비중" src="${imageURL + values.filePath}"/></div>`
      ...
      return `
      <img alt="이미지 준비중" src="${imageURL + values.filePath}"/>`
    })
